...
namespace VoxelBusters.Utility
{
	public partial class AndroidManifestGenerator 
	{
		...
		protected void WriteUsesPermission (XmlWriter _xmlWriter, string _name, Feature[] _features = null, string _comment = null, int maxSdkVersion = -1)
		{
			if (_comment != null)
				_xmlWriter.WriteComment(_comment);
			...
			_xmlWriter.WriteStartElement("uses-permission");
			{
				_xmlWriter.WriteAttributeString("android:name", 			_name);
				if(maxSdkVersion != -1)
					_xmlWriter.WriteAttributeString("android:maxSdkVersion", maxSdkVersion.ToString());
			}
			_xmlWriter.WriteEndElement();
