...
public class GoogleAnalyticsV4 : MonoBehaviour {
  ...
  void Awake() {
	...
	void OnDestroy(){
		LogEvent("Google Analytics", "Auto Instrumentation", "Game Close", 0);
	}
  ...
  private void errorHandler(string condition, string stackTrace, LogType type)
  {
      ...
      {
        string name = "Log-" + DateTime.Now.Month + "-" + DateTime.Now.Day + "-" + DateTime.Now.Hour + "-" + DateTime.Now.Minute + "-" + DateTime.Now.Second;

        LogEvent("Debug", name, (type == LogType.Error ? "Error" : "Exception"), 0);
        LogEvent("Debug", name, condition, 0);
        LogEvent("Debug", name, stackTrace, 0);
      }
  }
