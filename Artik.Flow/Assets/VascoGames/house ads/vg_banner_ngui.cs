...
public class vg_banner_ngui : MonoBehaviour {
	public Material tempmat;
	private string blink;
	private string spotid;
	private string adid;
	Texture2D bannerimg;
	public UITexture instImage;
...
//		instcanvas.SetActive(false);
	...
	IEnumerator makeclick()
	{
		...
		WWW www = new WWW(vg_interstitial.houseadslinkbanner + "/loadbannerrec.php?clicked=1&spotid=" + spotid + "&adid=" + adid + "&bid=" + vg_interstitial.GBundleId + "&deviceid=" + DeviceUniqueIdentifier.get());
		yield return www;
	...
	IEnumerator impressionok()
	{
		...
		WWW www = new WWW(vg_interstitial.houseadslinkbanner + "/loadbannerrec.php?impressieok=1&spotid=" + spotid + "&adid=" + adid + "&bid=" + vg_interstitial.GBundleId + "&deviceid=" + DeviceUniqueIdentifier.get());
		yield return www;
	}
	...
	public IEnumerator loadbanner(int start = 0)
	{
		...
		WWW www = new WWW(vg_interstitial.houseadslinkbanner + "/loadbannerrec.php?load=" + start + "&bid=" + vg_interstitial.GBundleId + "&deviceid=" + DeviceUniqueIdentifier.get());
		print(vg_interstitial.houseadslinkbanner + "/loadbannerrec.php?load=" + start + "&bid=" + vg_interstitial.GBundleId + "&deviceid=" + DeviceUniqueIdentifier.get());
		yield return www;

		XmlDocument	doc= new XmlDocument();
		doc.LoadXml(www.text);
		XmlNodeList bannerinfo = doc.SelectNodes("banner");
		Debug.Log("Install check " + bannerinfo[0].SelectSingleNode("packed").InnerText);
		if(bannerinfo[0].SelectSingleNode("packed").InnerText == "zero") {
			...
			instImage.mainTexture = bannerimg;
